		...
		$cn = $_POST['com_new'];
		 $ius=$_SESSION['id'];
          if ($wszystko_OK==true)
                    ...
                    if ($polaczenie->query("INSERT INTO `komentarze` (`id_kom`, `id_user`, `id_post`, `tresc`, `wyslano`,`status`) VALUES (NULL, '$ius', '$idposta','$cn',null,1);"))
					{
...
<?php				}else{ ?>
   ...
   <?php if(isset($_SESSION['ban'])){if($_SESSION['ban']==false){ ?><h2 class="error"> You are banned</h2><?php }} ?>
                       ...
                       ?>		
                                        <div class=" fic">
                                            <div class="pudelko_w_pudelku col-xl-12">
                                              <div class="row">   
                                            <h4 class="pudelko_na_tytul col-lg-10"><?php echo $row['title']?></h4>
    ...
    <?php
                ...
                {         $rezultat = $polaczenie->query("SELECT `id_kom`,`id_user`,`id_post`,`tresc`,`wyslano`,`status` FROM `komentarze`where id_post='$idposta'and status='2' ORDER BY `komentarze`.`wyslano` DESC");
                        
                            if (!$rezultat) throw new Exception($polaczenie->error);
                            $row_cnt = $rezultat->num_rows;
                            $pages = ceil($row_cnt/$count);
                          $rezultat2 =$polaczenie->query("SELECT `id_kom`,`id_user`,`id_post`,`tresc`,`wyslano`,`status` FROM `komentarze`where id_post='$idposta' and status='2' ORDER BY `komentarze`.`wyslano` DESC Limit $count offset $offset"); 
            ...
            ?>
                                        <div class=" fic">
                                            ...
                                            <h4 class="pudelko_na_tytul col-lg-11">Artykuł <?php echo $rezp['title']?></h4>
                                                    <?php if(isset($_SESSION['zalogowany'])){if((($rezu['username']==$_SESSION['user'])||($_SESSION['status']>1))&&($_SESSION['ban']!=false)){ ?>
                ...
                <?php
                   ...
                   <div class="pudelko_w_pudelku col-xl-6 offset-xl-3"><?php for($i=0;$i<$pages;$i++){
                    ...
                    echo '<a href="artykul.php?count='.$count.'&offset='.$i.'"> '.$i.' </a>';
                            ...
                            <div class="pudelko_na_artykuly offset-xl-1 col-sm-10 col-md-10">
        <h3 class="start col-xl-11 offset-xl-1" > nowe komentarze(oczekujące):</h3>
    <?php
                if ($polaczenie->connect_errno!=0)
                {
                    throw new Exception(mysqli_connect_errno());
                ...
                {         $rezultat = $polaczenie->query("SELECT `id_kom`,`id_user`,`id_post`,`tresc`,`wyslano`,`status` FROM `komentarze`where id_post='$idposta'and status='2' and id_user='$idurz'  ORDER BY `komentarze`.`wyslano` DESC");
                        
                            if (!$rezultat) throw new Exception($polaczenie->error);
                            $row_cnt = $rezultat->num_rows;
                            $pagesk = ceil($row_cnt/$countk);
                          $rezultat2 =$polaczenie->query("SELECT `id_kom`,`id_user`,`id_post`,`tresc`,`wyslano`,`status` FROM `komentarze`where id_post='$idposta' and status='1' and id_user='$idurz' ORDER BY `komentarze`.`wyslano` DESC Limit $countk offset $offsetk"); 
                         if (!$rezultat2) throw new Exception($polaczenie->error);
                            while($row = $rezultat2->fetch_assoc())
              { 
               $idu=$row["id_user"];
            ...
            ?>
                ...
                <?php
                   ...
                   <div class="pudelko_w_pudelku col-xl-6 offset-xl-3"><?php for($i=0;$i<$pagesk;$i++){
                    ...
                    echo '<a href="artykul.php?countk='.$countk.'&offsetk='.$i.'"> '.$i.' </a>';
                        }
                            }
                               ?> </div>   	
          </div></div>
