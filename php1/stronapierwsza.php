...
<?php				}else{ ?>
    ...
    <div class="container d-md-inline d-sd-inline">
        ...
        <div class="col-md-10 col-lg-5">
            ...
            <?php
                        ...
                        {        $rezultat = $polaczenie->query("SELECT uzytkownicy.username, posts.title,posts.article,posts.updated_at,posts.created_at,posts.akceptacja,posts.id_pos,posts.user_id_pos FROM `uzytkownicy`,`posts` WHERE uzytkownicy.id=posts.user_id_pos ORDER BY posts.created_at DESC");
    ...
    <?php
                ...
                {         $rezultat = $polaczenie->query("SELECT `id_user`,`id_post`,`tresc`,`wyslano`,`status` FROM `komentarze` WHERE `komentarze`.`status`='2' ORDER BY `komentarze`.`wyslano` DESC");
                            if (!$rezultat) throw new Exception($polaczenie->error);
                    $i=0;
                    while(($row = $rezultat->fetch_assoc())&&($i<4))
              { $i++;
               ...
               ?>		
                                <div class=" fic">
                                        ...
                                        <div class="row">
                                            ...
                                            <form action="texuser.php" method="post">
                                                <input name="useridpos" type="hidden" value="<?php echo $row["id"] ?>"> 
                                                <input name="submit" type="submit" value="o autorze" class="portol">
                                                </form>
                                            </div>
                                        </div>
